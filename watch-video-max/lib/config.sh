#!/usr/bin/env bash
# watch-video-max: portable configuration resolver.
#
# Source this file, do not execute it:
#   source /path/to/watch-video-max-skill/lib/config.sh
#
...
#
# This search order is shared with the sibling skill "watch-video" -- both packages read the
# same config file, so a value pinned once (e.g. WV_PYTHON) applies to both.
#
...
_wv_resolve WV_WORK_DIR      WV_WORK_DIR      "work_dir"      "" "${TMPDIR:-/tmp}/watch-video-max"
_wv_resolve WV_OUTPUT_DIR    WV_OUTPUT_DIR    "output_dir"    "" "$HOME/watch-video-max/reports"

# NOTE: recordings_dir / transcript_dir / delete_source used to resolve here, for the FORENSICS
# lens that this package no longer has. They belong to `video-autopsy` now and are read there.
# They may still be present in a shared family config file -- that is not an error and not a
# warning; this package simply does not consult them.
...
# This package's frame rate. The default is 5, NOT the light sibling's 1: measured 2026-08-29,
# three on-screen artifacts that were load-bearing to the content were illegible at 1 fps and
# readable at 5. Deep study is the whole point of this package, so its default has to be the rate
# that actually resolves what is on screen.
#
# The config key is `max_fps`, deliberately NOT `knowledge_fps`. `knowledge_fps` belongs to
# `watch-video`, which legitimately wants 1; sharing one key would force both packages to the same
# rate in a shared family config file and silently drop this one to the cheap default.
_wv_resolve WV_MAX_FPS WV_MAX_FPS "max_fps" "" "5"

# The 5 fps floor is ADVISORY-BUT-LOUD, not a silent clamp. A lower rate is allowed -- the user may
# have a good reason (a very long talking-head source, a tiny disk) -- but below 5 fps briefly-shown
# on-screen artifacts stop resolving, and any claim reconstructed from them becomes unsupportable.
# Warn on stderr so the run is never quietly downgraded, and require the report to say so.
if [ "$(awk -v v="${WV_MAX_FPS:-5}" 'BEGIN{print (v<5)?1:0}' 2>/dev/null)" = "1" ]; then
  echo "WARN  WV_MAX_FPS=${WV_MAX_FPS} is below the 5 fps rate this package is built around." >&2
  echo "      Briefly-shown on-screen artifacts (a command, a value, a slide) may fall between" >&2
  echo "      samples and never be captured at all." >&2
  echo "      Proceed only if the report header states the rate and marks artifact" >&2
  echo "      reconstruction as possibly incomplete." >&2
fi

export WV_FFMPEG WV_FFPROBE WV_PYTHON WV_YTDLP WV_JS_RUNTIME
export WV_WORK_DIR WV_OUTPUT_DIR
export WV_WHISPER_MODEL WV_WHISPER_DEVICE WV_WHISPER_COMPUTE WV_MAX_FPS

# ---------------------------------------------------------------------------------------------
# 6. transparency surface
# ---------------------------------------------------------------------------------------------

_WV_ALL_KEYS="WV_PYTHON WV_FFMPEG WV_FFPROBE WV_YTDLP WV_JS_RUNTIME WV_WORK_DIR WV_OUTPUT_DIR WV_WHISPER_MODEL WV_WHISPER_DEVICE WV_WHISPER_COMPUTE WV_MAX_FPS"

wv_config_dump() {
  echo "=== watch-video-max resolved configuration ==="
