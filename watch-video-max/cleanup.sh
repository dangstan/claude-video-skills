...
# are written, so a run that dies midway would otherwise leave its frames behind forever. This
# package extracts at 5 fps by default and can orphan tens of gigabytes.
#
...
WORK="${WV_WORK_DIR:-${TMPDIR:-/tmp}/watch-video-max}"
